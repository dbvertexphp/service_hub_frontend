...
const UplaodeVideo = () => {
  ...
  const UplaodS3Url = async () => {
    try {
      const response = await WebsiteApi.getReelsUploadUrlS3();
      if (response.message) {
  ...
  const onSubmit = async (e) => {
    ...
    if (!video) {
      validate = false;
      setVideoErr("Please Select Quicky.");
    }
    ...
    if (validate) {
      try {
        ...
        if (uplaodeThumImage == 200 && uplaodeVideofile == 200) {
          ...
          const reels_key = uplaodS3Url.videoget_url.key;
          response = await WebsiteApi.uploadReel(
            category_id,
            description,
            title,
            reels_key,
        ...
        <Grid container spacing={2} sx={{ justifyContent: "center" }}>
          <Grid item xs={8} className="" sx={{ alignSelf: "center" }}>
            <Stack direction="row" spacing={2} sx={{ justifyContent: "center", marginTop: "1vh" }}>
              <Typography className="Edit_profile_heading">Quicky</Typography>
            </Stack>
                    ...
                    <Box sx={{ ...commonStyles, position: "relative" }}>
                      {!video && (
                        <label htmlFor="videoInput">
                          ...
                          />
                          <br />
                          <typography className="Uplaod_icon_model_text">Upload Quicky</typography>
                          <input
                            type="file"
                            id="videoInput"
                            accept="video/*"
